...
from prediction_depth.knndnn import VGGPD, MLP7, ResNetPD, BasicBlockPD

parser = argparse.ArgumentParser(description='PyTorch Training')
parser.add_argument('--data-dir', default='dataset',
                    help='path to dataset')
parser.add_argument('--order-dir', default='balumba',
...
def main():
    ...
    if 'hsf' in args.order_dir:
        ...
        order = np.load('orders/angular_gap_order.npy')
        order = np.random.randint(0, order.max() - 1, len(order) // 2)
    ...
    model = ResNetPD(BasicBlockPD, [2, 2, 2, 2], temp=1.0, num_classes=10).cuda()
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model.state_dict(), f"cifar10_results_resnet/{args.arch}_cifar10sgd{seed}_.pt")
